...
import { removeCoinsByAdmin } from '../economy/repository.js';
...
export const usunCoinsyCommand = {
    data: new SlashCommandBuilder()
        .setName('usun-coinsy')
        .setDescription('➖ Usun cebuliony uzytkownikowi')
        .setDefaultMemberPermissions(null)
        .setDMPermission(false)
        .addUserOption((option) => {
            return option
                .setName('uzytkownik')
                .setDescription('Komu usunac coinsy')
                .setRequired(true);
        })
        .addIntegerOption((option) => {
            return option
                .setName('ilosc')
                .setDescription('Liczba coinsow do odjecia')
                .setMinValue(1)
                .setRequired(true);
        })
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        try {
            const mutation = await removeCoinsByAdmin({
                guildId,
            ...
            try {
                ...
                console.error('❌  Nie udalo sie zapisac logu /usun-coinsy:', error);
                loggingWarning = 'Operacja wykonana, ale nie udalo sie zapisac logu.';
            }

            const embed = new EmbedBuilder()
                .setColor(HusariaColors.RED)
                .setTitle('✅ Usunieto coinsy')
                .setDescription(`Usunieto **${amount}** coinsow uzytkownikowi <@${targetUser.id}>.`)
                .addFields(
            ...
            console.error('❌  Nie udalo sie wykonac /usun-coinsy:', error);
            await interaction.editReply({
                content: '❌ Wystapil blad podczas usuwania coinsow.',
                embeds: [],
            });
        }
    },
